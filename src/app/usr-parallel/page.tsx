...
export default async function Page({
  ...
  const postsData = getUserPosts(userId);//didn't use await to enable parallel
  const albumsData = getUserAlbums(userId);//didn't use await to enable parallel

  const [posts, albums] = await Promise.all([postsData, albumsData]);//which enable parallel
  return (
